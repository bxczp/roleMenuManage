...
public class UserController {
    ...
    // 使用 javax.validation.Valid 验证框架 和 org.springframework.validation.BindingResult
    // 验证信息
    public Map<String, Object> login(String imageCode, @Valid User user, BindingResult bindingResult,
        ...
        // @Valid 的验证信息 在 User实体类 中 定义 @NotEmpty(message="请输入用户名!")
    ...
    public JsonArray getAllMenuByParentId(Integer parentId, Integer roleId) {
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            if ("open".equals(jsonObject.get("state").getAsString())) { // 叶子节点
                continue;
            } else {
